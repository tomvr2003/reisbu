...
if(isset($_POST['submit_button'])) {
  ...
  $sql = "UPDATE reizen SET title=:title, omschrijving=:omschrijving, reisinfo=:reisinfo, star=:star, rating=:rating, image=:image WHERE id=:id";
  $statement = $conn->prepare($sql);
  $statement->execute([":title" => $title, ":omschrijving" => $omschrijving, ":reisinfo" => $reisinfo, ":star" => $star, ":rating" => $rating, ":image" => $image, ":id" => $id ]);
